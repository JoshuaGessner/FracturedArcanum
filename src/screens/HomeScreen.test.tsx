...
function buildShellValue(overrides: Partial<AppShellContextValue> = {}): AppShellContextValue {
  ...
  return {
    ...
    loggedIn: true,
    setupRequired: false,
    setupForm: { username: '', password: '', displayName: '' },
    setSetupForm: noop,
    setupError: '',
    setupLoading: false,
...
describe('HomeScreen navigation and footer', () => {
  ...
  it('navigates via the main home tiles', () => {
    const openScreen = vi.fn()
    renderHomeScreen({ openScreen })

    fireEvent.click(screen.getByRole('button', { name: /collection/i }))

    expect(openScreen).toHaveBeenCalledWith('collection')
  })
