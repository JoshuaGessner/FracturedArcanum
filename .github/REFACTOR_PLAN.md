...
> **Current state:** App.tsx = 3,156 lines, the provider tree and AppShellContext composition are fully live, all 7 screens and the supporting ceremony/onboarding overlays are in place, the SVG asset pipeline is verified at 173 generated assets, and release validation is green with 105 passing tests plus clean lint and build. Completed structural items are intentionally condensed so this plan stays focused on the shipped architecture and the remaining audit checklist.
...
> Note: the legacy quick-emote surface was intentionally removed earlier in Phase 1G. The rest of the Phase 3 polish shipped without re-introducing a standing emote bar, keeping battle readability and mobile density intact.
...
> This backlog is now preserved as a completion record. The high-value Phase 3 polish and stability steps listed here have been implemented and verified.

| Step | Description | Risk | Primary files | Status |
|------|-------------|------|---------------|--------|
| **3D** | Finish transition and motion cleanup where screen changes still feel web-like instead of game-like | Medium | App.tsx, App.css | 🟢 done (curtain, rune-wipe, portal, modal, and reduced-motion transition cleanup all landed) |
| **3E** | Tighten Home dashboard so hero, tiles, and utility cards fit the viewport more effectively | High | HomeScreen.tsx, App.css | 🟢 done |
| **3F** | Compress Play screen and queue states so the central CTA remains above the fold | Medium | PlayScreen.tsx, App.css | 🟢 done |
| **3G** | Keep Collection filters and deck tools visible while making the card browser the intentional scroll region | Medium | CollectionScreen.tsx, App.css | 🟢 done |
| **3H** | Convert Social into denser, more segmented panels to reduce page-length sprawl | Medium | SocialScreen.tsx, App.css | 🟢 done |
| **3I** | Keep Shop purchase actions and reward state visible without long stacked scrolling | Medium | ShopScreen.tsx, App.css | 🟢 done |
| **3J** | Final battle layout compression pass for one-screen mobile combat | Highest | BattleScreen.tsx, App.css, App.tsx | 🟢 done (board-first combat view, slim duel ribbon, compact dock, horizontal hand fan) |
| **3K** | Compact Settings and admin surfaces into grouped, tabbed, or sectional layouts | Medium | SettingsScreen.tsx, App.css | 🟢 done |
| **3L** | Finish remaining gamification and ceremonial polish where it still supports readability | Low | App.css, overlays, screens | 🟢 done |
| **3M** | Full-viewport density pass across all player-facing screens | Highest | App.css, all screen files | 🟢 done (primary scenes now fit the viewport as intended, with the content region owning scroll) |
| **3N** | Harden long-press inspect and suppress browser/system context menus | Medium | App.tsx, App.css, BattleScreen.tsx | 🟢 done |
| **3O** | Normalize button/panel chrome and remove stray floating frame artifacts | Highest | App.css, NavBar.tsx, HomeScreen.tsx, BattleScreen.tsx | 🟢 done |
| **3P** | Stabilize reconnect/rejoin recovery so live play does not require refresh | Highest | App.tsx, server/server.js, server/game-room.js | 🟢 done (client and server recovery flow audited, idempotent, and verified) |
| **3Q** | One-screen scene shell — no body scroll on primary screens, scene-body owns scroll | Highest | App.tsx, App.css, all screen files | 🟢 done |
| **3R** | Hearthstone-style scene transitions (curtain / reverse / portal slam / rune wipe / modal materialize) + paired sounds | High | audio.ts, App.css, App.tsx, utils.ts | 🟢 done |
| **3S** | Battle drag-to-play, attack arrow telegraph, hero portrait reactions, low-HP heartbeat | High | BattleScreen.tsx, App.tsx, App.css, audio.ts | 🟢 done |
| **3T** | Full pack opening ceremony overlay with rarity-tiered glow + sound | Medium | new PackCeremonyOverlay.tsx, ShopScreen.tsx, App.css, audio.ts | 🟢 done |
| **3U** | Unified `feedback()` primitive for sound + haptic + flash on all controls | Medium | new feedback.ts, App.tsx, all screens | 🟢 done |
...
| **3W** | Unified `RewardCinemaOverlay` for battle/daily/pack/rank-up reveal beats | Medium | new RewardCinemaOverlay.tsx, RewardCinemaSequence.ts, App.tsx, ShopScreen.tsx, App.css | 🟢 done |
| **3X** | First-launch onboarding tour with spotlight cutouts; replay from Settings | Medium | new OnboardingTour.tsx, App.tsx, SettingsScreen.tsx | 🟢 done |
| **3Y** | Touch swipe gesture between adjacent primary scenes, with rail-aware guards | Medium | App.tsx, App.css, utils.ts | 🟢 done |
